...
async function importContacts() {
  try {
      ...
      try {
        ...
        await prisma.contact.create({
          data: {
            ...
            account: {
              connectOrCreate: {
                where: { id: 'default-account-' + organization.id },
                create: {
                  id: 'default-account-' + organization.id,
        ...
        // If it's a unique constraint error, skip silently
        if (error.code === 'P2002') {
    ...
    console.log(`⏭️  Skipped (duplicates/errors): ${skipped} contacts`);
    if (errors.length > 0) {
